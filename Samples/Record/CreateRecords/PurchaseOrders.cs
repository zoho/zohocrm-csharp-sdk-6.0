...
namespace Samples.Record.CreateRecords
{
	...
	{
		public static void CreatePurchaseOrders_1(string module)
		{
			...
			record1.AddFieldValue(Com.Zoho.Crm.API.Record.Purchase_Orders.VENDOR_NAME, vendor);
			record1.AddFieldValue(Com.Zoho.Crm.API.Record.Purchase_Orders.SUBJECT, "new order");
			record1.AddFieldValue(Com.Zoho.Crm.API.Record.Purchase_Orders.ADJUSTMENT, 30.0);
			record1.AddFieldValue(Com.Zoho.Crm.API.Record.Purchase_Orders.CARRIER, new Choice<string>("FedEx"));
			record1.AddFieldValue(Com.Zoho.Crm.API.Record.Purchase_Orders.PO_DATE, new DateTime(2023, 9, 23).Date);
			record1.AddFieldValue(Com.Zoho.Crm.API.Record.Purchase_Orders.PO_NUMBER, "123123");
			record1.AddFieldValue(Com.Zoho.Crm.API.Record.Purchase_Orders.EXCISE_DUTY, 25.0);
			Com.Zoho.Crm.API.Record.Record vendor_name =  new Com.Zoho.Crm.API.Record.Record();
			vendor_name.AddFieldValue(Com.Zoho.Crm.API.Record.Vendors.ID, 3421423242421L);
			record1.AddFieldValue(Com.Zoho.Crm.API.Record.Purchase_Orders.VENDOR_NAME, vendor_name);
			record1.AddFieldValue(Com.Zoho.Crm.API.Record.Purchase_Orders.TRACKING_NUMBER, "42231");
			record1.AddFieldValue(Com.Zoho.Crm.API.Record.Purchase_Orders.SALES_COMMISSION, 43.32);
			record1.AddFieldValue(Com.Zoho.Crm.API.Record.Purchase_Orders.REQUISITION_NO, null);
			Com.Zoho.Crm.API.Record.Record contact_Name =  new Com.Zoho.Crm.API.Record.Record();
			contact_Name.AddFieldValue(Com.Zoho.Crm.API.Record.Contacts.ID, 342342322231L);
			record1.AddFieldValue(Com.Zoho.Crm.API.Record.Purchase_Orders.CONTACT_NAME, contact_Name);
			record1.AddFieldValue(Com.Zoho.Crm.API.Record.Purchase_Orders.DESCRIPTION, "description");
			record1.AddFieldValue(Com.Zoho.Crm.API.Record.Purchase_Orders.TERMS_AND_CONDITIONS, "details of terms and condition");
			...
			lineItemProduct.Id = 4402480954344l;
	...
	//		inventoryLineItem.AddKeyValue("Product_Category_1", "hardware");
			...
			productLineTax.Name = "Vat";
			productLineTax.Value = 10.0;
			productLineTax.Id = 4402480020810l;
			productLineTax.Percentage = 15.0;
			productLineTaxes.Add (productLineTax);
			inventoryLineItem.AddKeyValue("Line_Tax", productLineTaxes);
			inventoryLineItemList.Add (inventoryLineItem);
			record1.AddFieldValue(Com.Zoho.Crm.API.Record.Purchase_Orders.PURCHASE_ITEMS, inventoryLineItemList);
			...
			// for Custom LookUp
			Com.Zoho.Crm.API.Record.Record data1 =  new Com.Zoho.Crm.API.Record.Record();
			data1.Id = 440248774074L;
			record1.AddKeyValue("Lookup_2", data1);
			// for Custom pickList
		...
		public static void Call()
		{
			try
			{
                ...
                CreatePurchaseOrders_1(moduleAPIName);
			}
			catch (Exception e)
			{
