...
using Com.Zoho.Crm.API.Timelines;
...
namespace Com.Zoho.Crm.API.Util
{
    ...
    public class DataTypeConverter<T>
    {
		...
		private static void Init()
		{
			...
			AddToDictionary(typeof(TimeSpan).FullName,
			(obj) =>
			{
				string[] timeValue = obj.ToString().Split(':');
				TimeSpan timeSpan = new TimeSpan(int.Parse(timeValue[0]), int.Parse(timeValue[1]), 00);
                return (T)Convert.ChangeType(timeSpan, typeof(T));
            },
			(obj) =>
			{
				TimeSpan timeSpan = (TimeSpan)Convert.ChangeType(obj, typeof(T));
			...
			AddToDictionary(typeof(int).FullName,
			(obj) =>
			{
			...
			AddToDictionary(typeof(TimeZoneInfo).FullName,
            (obj) =>
			{
				try
				{
                    TimeZoneInfo timeZone = TimeZoneInfo.FindSystemTimeZoneById(obj.ToString());
                    return (T)Convert.ChangeType(timeZone, typeof(T));
                }
                catch(Exception)
				{
					TimeZoneInfo timeZone = GetTimeZone(obj.ToString());
                    return (T)Convert.ChangeType(timeZone, typeof(T));
                }
			},
            (obj) =>
			{
                TimeZoneInfo timeZone = (TimeZoneInfo)Convert.ChangeType(obj, typeof(T));
				return timeZone.Id;
            });
		}

		private static TimeZoneInfo GetTimeZone(string name)
		{
			foreach(TimeZoneInfo timezone in TimeZoneInfo.GetSystemTimeZones())
			{
				if(timezone.Id.Equals(name) || timezone.StandardName.Equals(name))
				{
					return timezone;
                }
			}
